...
static RegisterPass<FlowDependenceAnalysis> FDA("fda", "Flow  Dependence Analysis", false, true);
...
std::vector<Instruction*> FlowDependenceAnalysis::getDependencyInFunction(Function &F, Instruction* pLoadInst){
  ...
  AliasAnalysis::Location LI_Loc = ProgramDependencyGraph::Global_AA->getLocation(LI);
  ...
  for(int j = 0; j < StoreVec.size(); j++)
    {
      ...
      AliasAnalysis::Location SI_Loc = ProgramDependencyGraph::Global_AA->getLocation(SI);

      //      errs() << "SI =" << *SI << ' ';
      //errs() << "SI.Loc = " << SI_Loc.Ptr << " LI.Loc = " << LI_Loc.Ptr << ' ';
  
      AliasAnalysis::AliasResult AA_result = ProgramDependencyGraph::Global_AA->alias(LI_Loc, SI_Loc);

      //      errs() << "@@@@@@Test Global AA :" << LI_Loc.Ptr << '\n';



      if(AA_result != AliasAnalysis::NoAlias){
	//errs() << "store instruction alias found! SI = " << *SI << " LI = " << *LI << '\n';
...
FlowDepResult FlowDependenceAnalysis::
...
isLoadLoadClobberIfExtendedToFullWidth(const AliasAnalysis::Location &MemLoc,
  ...
  unsigned Size = FlowDependenceAnalysis::
...
unsigned FlowDependenceAnalysis::
...
getPointerDependencyFrom(const AliasAnalysis::Location &MemLoc, bool isLoad,
  ...
  while (ScanIt != BB->begin()) {
    ...
    if (LoadInst *LI = dyn_cast<LoadInst>(Inst)) {
      ...
      /// Zhiyuan: ? Is this the reason why there are too many clobbers in the ToyPointer code?
      /// Zhiyuan ? what is unordered?
      if (!LI->isUnordered())
	  ...
	  /// FIXME: Zhiyuan: there is no data dependence between QueryInst and Inst according to our definition.
	  /** it is a read after read (RAR).
	   *  (1) a = *p;
	   *  (2) q = p;
	   *  (3) b = *p;
	   *  (1) and (3) are both Load instruction, they do not have "Def" relation inside.
	   */
          return FlowDepResult::getDef(Inst);
        }
      ...
      /// Zhiyuan: Since we have already handled LOAD instruction, now we handle STORE instruction.
      /// Store after Load (WAR) => Anti-dependency
...
FlowDepResult FlowDependenceAnalysis::getDependency(Instruction *QueryInst) {
  ...
  /// LocalDeps[QueryInst] will call the constructor where dirty is set to be true.
  ...
  if (BasicBlock::iterator(QueryInst) == QueryParent->begin()) {
    //    errs() << "**** BasicBlock::iterator(QueryInst) == QueryParent->begin()\n";
    //    errs() << "**** BasicBlock::iterator(QueryInst) =" << *BasicBlock::iterator(QueryInst) << "\n";
    //    errs() << "**** QueryParent->begin()            =" << *QueryParent->begin() << "\n";
    /// Zhiyuan: if this is the first instruction of the basic block.
    // No dependence found.  If this is the entry block of the function, it is
    // unknown, otherwise it is non-local.
    //    errs() << "**** QueryParent                                 =" << *QueryParent << "\n";
    //    errs() << "**** QueryParent->getParent()->getEntryBlock()   =" << QueryParent->getParent()->getEntryBlock() << "\n";
    ...
    /// Zhiyuan: use AliasAnalysis's ModRefResult to check the Mod/Ref info between the instruction and the memory it accesses.
    AliasAnalysis::ModRefResult MR = GetLocation(QueryInst, MemLoc, AA);
    /// Zhiyuan: if the memory location is not null (that means the instruction actually access memory.)
    if (MemLoc.Ptr) {
      // If we can do a pointer scan, make it happen.
      /// AliasAnalysis::Mod == 0x10, when MR == NoModRef or Ref, isLoad == 1, when MR == Mod or ModRef, isLoad == 0
...
getNonLocalPointerDepFromBB(const PHITransAddr &Pointer,
  ...
  while (!Worklist.empty()) {
    ...
    if (!SkipFirstBlock) {
      ...
      DEBUG(AssertSorted(*Cache, NumSortedEntries));
      FlowDepResult Dep = GetNonLocalInfoForBlock(Loc, isLoad, BB, Cache,
						  NumSortedEntries);
...
void FlowDependenceAnalysis::removeInstruction(Instruction *RemInst) {
  ...
  if (!RemInst->isTerminator())
    NewDirtyVal = FlowDepResult::getDirty(++BasicBlock::iterator(RemInst));
...
/*
  ...
  for (SmallVector<NonLocalDepResult, 20>::iterator it = Result.begin(); it != Result.end(); it++) {
  NonLocalDepResult nld = *it;
  errs() << "Inst: " << nld.getResult().getInst() << "\n";
  }
  }//end if MemLoc.Ptr
  }//end for
